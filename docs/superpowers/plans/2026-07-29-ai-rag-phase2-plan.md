...
|         API 离线合同         |                                        Nitro 路由、鉴权/错误映射、同步记录 schema、pgvector migration、Hybrid Search 注入合同已落地；凭据守卫显式扫描 `drizzle`、`server`、`src`、发布配置与 `.sql` migration，不扫描测试正则字面量                                        |                                                                                    `pnpm --filter @ruan-cat-drill-doc/ai-rag-api test`：`12` 文件、`35` 用例通过；类型检查与 `build:vercel`                                                                                    |                      没有真实 PostgreSQL lexical/vector provider                      |
...
|         本地 Chat UI         | `AiChat` 已直接接入 `Bubble`、`BubbleList`、`Sender` 与 `MarkdownRender`；助手消息采用 `mode="chat"`、`html-policy="escape"`，默认使用 `smoothStreaming="auto"` 与 `typewriter`、固定 `fade=false`，减少动态效果时关闭三项；外部流式响应期提供可见、可访问的“停止生成”入口 |                   `pnpm --filter @ruan-cat-drill-doc/ai-vue test`：`4` 文件、`15` 用例通过；其中真实 `markstream-vue` DOM 验收覆盖表格、未闭合 fence、20k/1k 行长文、HTML/`javascript:` XSS 与 `final` 更新；`typecheck`、`build` 通过；停止入口独立复核通过                   |     尚未完成真实浏览器、SSR hydration、真实 transport 中止与逐 token 代码高亮验收     |
|     本地 Chat transport      |                                         `@ai-sdk/vue@1.2.12` 仅装配于 `ai-vitepress-plugins`；来源帧只传递 `id`、`label`、`sourceHref`，每轮请求清空 SDK `data` 并以新助手消息 ID 隔离来源；`503 RAG_NOT_CONFIGURED` 可展示和关闭                                          |                                                          `ai-vitepress-plugins` 全包测试 `2` 文件、`6` 用例通过，含 transport 的 `3` 个用例；API 路由合同、三包 typecheck 与 `git diff --check` 通过；独立复核已通过                                                           |               未验证真实 HTTP data-stream 时序、模型、数据库或生产装配                |
|        VitePress 锚点        |                                                                                                                    构建期稳定锚点与来源 URL 映射已落地                                                                                                                     | 当前最终暂存状态下再次执行 `$env:NODE_OPTIONS='--max-old-space-size=8192'; pnpm run docs:build`，退出码 `0`，输出 `9 successful, 9 total`；完整日志：`.superpowers/sdd/2026-07-29-ai-rag-phase2-plan/docs-build-2026-08-03-final.log`；`docs/.vitepress/dist` 含 `6600` 个文件 |                                 外部部署回归仍未执行                                  |
...
|   P1   |                        诊断并通过完整 `pnpm run docs:build`                        |                                可复现当前文档构建环境                                |               当前最终暂存状态下完整构建退出码 `0`、`9 successful, 9 total`、`6600` 个产物文件及完整日志               |           本地完成；外部部署未验证            |
...
- 2026-08-03：在当前暂存代码上重新验证本地包边界。`pnpm --filter @ruan-cat-drill-doc/ai-rag-api test` 为 `12` 文件、`35` 用例通过，`ai-vitepress-plugins` 为 `2` 文件、`6` 用例通过，`ai-vue` 为 `4` 文件、`15` 用例通过；三个包的 `typecheck` 均退出码 `0`，`git diff --cached --check` 无输出。此次验证不覆盖真实浏览器、真实 HTTP data-stream/abort、根 `pnpm run docs:build` 的当前暂存状态，也不改变 PostgreSQL、embedding、同步事务、生产装配、Vercel 部署和演示视频的外部门禁。
- 2026-08-03：在无开发服务器、无并发构建的条件下，以 `NODE_OPTIONS=--max-old-space-size=8192` 对当前最终暂存状态重新执行根 `pnpm run docs:build`。命令退出码为 `0`，输出 `9 successful, 9 total`，`docs/.vitepress/dist` 检查到 `6600` 个文件；完整 stdout/stderr 已保存至 `.superpowers/sdd/2026-07-29-ai-rag-phase2-plan/docs-build-2026-08-03-final.log`。该证据只覆盖本地当前构建，不代表 Neon、PostgreSQL、embedding、模型服务、真实浏览器、Vercel 部署或演示视频已完成。
