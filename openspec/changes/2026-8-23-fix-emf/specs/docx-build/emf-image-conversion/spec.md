...
#### Scenario: 归档后仍可复跑全量审计基线

- **WHEN** OpenSpec change 已归档或 evidence 文件已迁移，开发者在具备完整 DOCX 源目录的本机运行全量审计测试
- **THEN** 测试从转换子包受版本控制的 fixture 清单读取预期媒体标识、长度、SHA-256 与 record 审计结果
- **AND** 测试重新从当前源 DOCX 提取媒体并断言其内容与清单一致，不得读取或硬编码 OpenSpec evidence 路径
- **AND** 缺少完整源目录时专用全量命令必须明确失败，不得跳过后报告通过

#### Scenario: 全量 SVG 转换只验证可自动证明的契约

- **WHEN** 全量审计测试遍历清单中的唯一 EMF/WMF 二进制载荷
- **THEN** 每个载荷的 SVG 转换必须返回带 `viewBox` 的非空根 `<svg>`，并不得退化成唯一全画布 PNG `<image>`
- **AND** 每个清单引用都可回溯到已验证的 SHA-256 转换结果
- **AND** 此测试不得将上述结构断言解释为乱码、错位、重复、裁断或占位符的视觉通过

#### Scenario: ROP3 DIB 掩膜不在 SVG 裁剪边界泄漏为框体外点阵

- **WHEN** EMF 使用成对的 `EMR_STRETCHDIBITS`，分别声明 `SRCAND`（`0x008800C6`）和 `SRCPAINT`（`0x00EE0086`）以合成带边框的图元
- **THEN** SVG 转换仅对相邻、同目标区域且 ROP 互补的该受限组合跳过错误的 `<image>/<use>` 掩膜输出，保留其余 GDI 向量记录
- **AND** SVG 中框体下方的 GDI+ 空白带不出现蓝色、黑色或白色稀疏点阵，PNG 默认尺寸与既有契约不变
- **AND** 不在本场景中的 ROP3 值继续保留为未验证风险，不得因点阵消失而宣称通用 ROP3 保真
