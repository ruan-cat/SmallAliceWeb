...
- **resolved**：Production API 的真实 `OPTIONS /v1/chat` 初次返回了 CORS 头但状态为 404，不能满足浏览器 preflight。新增 CORS middleware 后，纯函数回归固定为 204、`Access-Control-Allow-Origin: *`、`POST` 与 `content-type`；本地 typecheck 和 Vercel build 通过，仍待 main 生产域名再验。
- **active**：`main@05e7887` 已证实 CORS/跨项目地址解析正常：Chrome 的 `/v1/chat` 请求渲染 5 个生产来源卡片，API 也返回 200 data-stream；但随后 stream error frame 为 `An error occurred.`，没有助手内容，故还不能验收停止/内容保留。新增 `streamText.onError` 仅记录错误消息（不记录 API key 或请求内容），下一轮 Production runtime log 是定位上游模型/流协议问题的依据。
