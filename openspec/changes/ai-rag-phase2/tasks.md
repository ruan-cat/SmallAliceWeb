...
- [ ] 2.1.4 [browser/e2e] 文档站 + 生产 Nitro - 生产后端驱动浏览器回归
  ...
  - 2026-08-25 Production 浏览器证据与阻塞：环境纠偏后的 `main@0abcd39` Nitro deployment `dpl_FucQX1HR6F3uWLNw2qcg6B48a2dt` Ready，正式自定义域名已指向该 deployment。真实页面请求进入 streaming，展示 5 个来源和停止入口；点击停止后约 52.5 秒的 `/v1/chat` 请求结束，停止/Loading 消失、输入恢复、来源保留。来源“运行环境”成功跳转到确定性 `#rag-heading-HxfmfBGZqpL4PlJbR1hlCj7ek5vi8CenAqlD09oZeg4`，目标 DOM id 存在且页面滚动至 `scrollY=836`。但 Production Nitro 60 秒、直接上游最小 `/v1/responses` 45 秒、真实浏览器约 52.5 秒三条路径均未收到 HTTP/SSE 文本增量或终止事件；阻塞指纹为 `api.code-tab.com /v1/responses + gpt-5.6-luna + stream:true 无事件直到客户端 abort`，尝试次数 3。按硬暂停规则停止重试；需要上游供应商恢复 Responses SSE 或提供可返回事件的同协议模型后，才能补齐首段/终止事件并完成本任务。
