...
Git Integration 部署监听固定使用本机全局 Vercel CLI：先执行 `vercel --version` 确认已安装版本，再对目标 deployment 执行 `vercel inspect <deployment-id-or-url> --wait --timeout 3m`，随后执行 `vercel inspect <deployment-id-or-url> --logs`。禁止为验收临时执行 `pnpm dlx vercel@latest` 下载 latest，以免 CLI 自身依赖解析漂移；只有 CLI 明确返回 `status Ready`、匹配 Git checkout SHA 且日志出现构建与部署完成，才允许进入生产浏览器验收；`QUEUED`/`BUILDING` 只能作为进行中状态。
