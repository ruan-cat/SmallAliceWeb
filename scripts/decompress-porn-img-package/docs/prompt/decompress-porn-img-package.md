...
比如 `D:\store\baidu\315.咬一口兔娘` 目录，有很多文件夹。但是处理起来我只需要处理少部分的文件夹，按照递增连号顺序处理。

该任务需要你在一定程度内整理重构 `scripts\decompress-porn-img-package` 包。请你认真做好改动调研。

### 智能识别并整理文件夹目录层级

比如我配置 213 ~ 220 数目，那么就进入到这些文件夹，并且按照不同的阶段完成处理。识别其阶段，并开始整理图片，并最后实现减少文件夹层级的需求。这些是已经解压缩后的文件夹，但是没有整理好文件夹的目录层级，需要你智能识别并整理。

### 智能识别并解压缩包

比如我配置 221 ~ 228 ，那么就检查这些文件，发现是压缩包，就按照默认的处理行为逐步处理。

## 08 <!-- TODO: --> 更改在根目录内设置目录的逻辑，更换新的目录名称作为根目录的文件夹名称

针对 `scripts\decompress-porn-img-package` 这款包的逻辑。

比如这款地址： `D:\store\baidu\315.咬一口兔娘\229\229\咬一口兔娘(Yiko湿润兔) – NO.229 5月 查无此人-简·杜 Mr.TAXI[142P-4V-3.3G]`

工作目录是 `D:\store\baidu\315.咬一口兔娘` 。按照目前的代码逻辑，最终输出到根目录的文件夹名称，是 229。是一串数字，是不具有语义化含义的目录名称。

我希望更改逻辑，在工作目录的根目录内新建文件夹时，其文件夹的命名取用至最后一个解压的最底层的文件夹名称。比如根据该例子，我期望的目标文件夹名称应该是 `咬一口兔娘(Yiko湿润兔) – NO.229 5月 查无此人-简·杜 Mr.TAXI[142P-4V-3.3G]` 。

那么最后的产物的目录应该是： `D:\store\baidu\315.咬一口兔娘\咬一口兔娘(Yiko湿润兔) – NO.229 5月 查无此人-简·杜 Mr.TAXI[142P-4V-3.3G]`。
